/*
Found at: http://www.senocular.com/pub/javascript/CSS_parse.js
...
class CSS {
  ...
  static function whiteSpaceToSpaces(string) {
	  return str.replace(/\s+/g," ");
  }
  
  static function removeComments(str) {
	  str = str.replace(/<!--/g,"");
	  str = str.replace(/-->/g,"");
	  return str.replace(/\/\*(\r|\n|.)*\*\//g,"");
  }

  static function arrayContains(ary, value) {
	  var i, len = ary.length;
	  for (i=0; i<len; i++) if (ary[i] == value) return true;
	  
	  return false;
  }
  
  static function arrayContainsEachhaystack_ary, needles_ary) {
  ...
  static function combine(target, copyFrom){
	  for(var p in copyFrom) target[p] = copyFrom[p];
  }


  public function new(style_str, dom) {
	  this.data = "";
